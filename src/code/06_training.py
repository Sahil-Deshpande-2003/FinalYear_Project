...
def attention_map(inpt, channels):
    ...
    roi_map = tf.constant(roi_map_value, dtype=tf.float32)
...
if __name__=="__main__":
    ...
    def saveBestModel(epoch, logs):
        global best_val_acc
        val_acc = logs.get('val_accuracy')  # Get validation accuracy from logs
        if val_acc is not None and val_acc > best_val_acc:
            print(f"Save model, val_acc={val_acc}")
            best_val_acc = val_acc
            model.save(save_path + 'df_ytb_Meso2.h5')
